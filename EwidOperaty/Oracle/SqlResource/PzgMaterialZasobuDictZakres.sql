SELECT 
    ...
    (SELECT cel_id FROM (SELECT oc.idop, wm_concat(oc.cel_id) cel_id FROM osr_operat_cel oc GROUP BY oc.idop) cele WHERE cele.idop = op.idop) pzg_cel,
	(SELECT dlk FROM (SELECT idop, wm_concat(dlk) dlk FROM (select distinct od.idop, SUBSTR(EW_DLK_IDG5(d.mslink), 15, 10) dlk from osr_operat_dlk od join kdzialka d using(dzialka_id) where od.stan_po = 0 union select distinct od.idop, SUBSTR(EW_DLK_IDG5(d.mslink), 15, 10) dlk from osr_operat_dlk od join kdzialka_a d using(dzialka_id) where od.stan_po = 0) GROUP BY idop) dz WHERE dz.idop = op.idop) dzialkaPrzed,
